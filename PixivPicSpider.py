...
class Pixiv():
    ...
    def get_img(self, html, date):
        ...
        for section in section_list:
            rank = section['data-rank-text']  #  排名
            title = section['data-title']  #  标题
            name = section['data-user-name']  #  作者
            data_id = section['data-id']  #  图片
            ...
            save_name = rank + '--' + title + '--' + name
            img_referer = self.img_referer + data_id  # 作为下载原图添加的referer
            href_temp = section.find('img')
            href = href_temp['data-src']  #  由此改造成原图的地址
            try:
                page_count_temp = section.find('div', attrs = {'class' : 'page-count'})
                page_count = int(page_count_temp.span.string)
                print('创建名为' + save_name + '的文件夹')
                os.makedirs(os.path.join(self.load_path, date, save_name))
                os.chdir(os.path.join(self.load_path, date, save_name))
                for i in range(0, page_count):
                    url = href.replace('c/240x480/img-master', 'img-original').replace('_master1200', '').replace('p0', 'p' + str(i))  #  原图地址
                    save_name = 'p' + str(i)
                    self.download_img(url, img_referer, date, save_name)
                    if i == page_count - 1:
